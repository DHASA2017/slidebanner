...
public class DLBannerView<T> extends RelativeLayout {
    ...
    public DLBannerView(@NonNull Context context) {
        super(context);
        init();
    }

    public DLBannerView(@NonNull Context context, @Nullable AttributeSet attrs) {
        super(context, attrs);
        readAttrs(context,attrs);
        init();
    }

    public DLBannerView(@NonNull Context context, @Nullable AttributeSet attrs, @AttrRes int defStyleAttr) {
    ...
    public DLBannerView(@NonNull Context context, @Nullable AttributeSet attrs, @AttrRes int defStyleAttr, @StyleRes int defStyleRes) {
    ...
    private void readAttrs(Context context,AttributeSet attrs){
        TypedArray typedArray = context.obtainStyledAttributes(attrs,R.styleable.DLBannerView);
        mIsOpenMZEffect = typedArray.getBoolean(R.styleable.DLBannerView_open_mz_mode,true);
        mIsCanLoop = typedArray.getBoolean(R.styleable.DLBannerView_canLoop,true);
    ...
    /**
     * 开始轮播
     * <p>应该确保在调用用了{@link DLBannerView {@link #setPages(List, DLHolderCreator)}} 之后调用这个方法开始轮播</p>
     */
    public void start(){
     ...
     * @param DLHolderCreator  ViewHolder生成器 {@link DLHolderCreator} And {@link DLViewHolder}
     */
    public void setPages(List<T> datas,DLHolderCreator DLHolderCreator){
        if(datas == null || DLHolderCreator == null){
            return;
        }
        mDatas = datas;
        ...
        mAdapter = new MZPagerAdapter(datas, DLHolderCreator,mIsCanLoop);
    ...
    public static class  MZPagerAdapter<T> extends PagerAdapter{
        private List<T> mDatas;
        private DLHolderCreator mDLHolderCreator;
        private ViewPager mViewPager;
        private boolean canLoop;
        private BannerPageClickListener mPageClickListener;

        public MZPagerAdapter(List<T> datas, DLHolderCreator DLHolderCreator, boolean canLoop) {
            if(mDatas == null){
           ...
           // mDatas.add(datas.get(0));//在最后加入最前面一个
            mDLHolderCreator = DLHolderCreator;
        ...
        private View getView(int position,ViewGroup container){
            ...
            DLViewHolder holder =null;
            // create holder
            holder = mDLHolderCreator.createViewHolder();
            ...
            if( mDatas!=null && mDatas.size()>0){
                holder.onBind(container.getContext(),realPosition,mDatas.get(realPosition),mDatas);
            }
